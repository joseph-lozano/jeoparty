defmodule Jeoparty.CluesTest do
  use Jeoparty.DataCase

  alias Jeoparty.Clues
  alias Jeoparty.Clues.Clue
  ...
  def fixture(:clue, attrs \\ @create_attrs) do
    {:ok, clue} = Clues.create_clue(attrs)
    clue
  end

  test "list_clues/1 returns all clues" do
    clue = fixture(:clue)
    assert Clues.list_clues() == [clue]
  end

  test "get_clue! returns the clue with given id" do
    clue = fixture(:clue)
    assert Clues.get_clue!(clue.id) == clue
  end

  test "create_clue/1 with valid data creates a clue" do
    assert {:ok, %Clue{} = clue} = Clues.create_clue(@create_attrs)
  ...
  test "create_clue/1 with invalid data returns error changeset" do
    assert {:error, %Ecto.Changeset{}} = Clues.create_clue(@invalid_attrs)
  end

  test "update_clue/2 with valid data updates the clue" do
    clue = fixture(:clue)
    assert {:ok, clue} = Clues.update_clue(clue, @update_attrs)
    assert %Clue{} = clue
  ...
  test "update_clue/2 with invalid data returns error changeset" do
    clue = fixture(:clue)
    assert {:error, %Ecto.Changeset{}} = Clues.update_clue(clue, @invalid_attrs)
    assert clue == Clues.get_clue!(clue.id)
  end

  test "delete_clue/1 deletes the clue" do
    clue = fixture(:clue)
    assert {:ok, %Clue{}} = Clues.delete_clue(clue)
    assert_raise Ecto.NoResultsError, fn -> Clues.get_clue!(clue.id) end
  end

  test "change_clue/1 returns a clue changeset" do
    clue = fixture(:clue)
    assert %Ecto.Changeset{} = Clues.change_clue(clue)
  end
end
